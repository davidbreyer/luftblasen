...
function playSuccessSound(hasStreakBonus = false) {
  ...
  [
    ...
    playTone({ frequency, start: now + delay, duration: 0.18, type: "triangle", peak: 1.05, destination: master });
  });

  if (hasStreakBonus) {
    playTone({ frequency: 1046.5, start: now + 0.32, duration: 0.26, type: "sine", peak: 0.98, destination: master });
  }
}
...
function playLifeLostSound() {
  ...
  master.connect(context.destination);
  playTone({ frequency: 220, endFrequency: 95, start: now, duration: 0.28, type: "sawtooth", peak: 1, destination: master });
}
...
function playLowTimeTick() {
  ...
  master.connect(context.destination);
  playTone({ frequency: 880, endFrequency: 660, start: now, duration: 0.07, type: "square", peak: 0.92, destination: master });
}
